...
export default class CreateAccountContractComponent extends LightningElement {
    ...
    @track isModalOpen = true; // Controls modal visibility
    ...
    loadClauses() {
        getAccountClauses()
            .then(result => {
                this.contractClauses = result.map(clause => ({
                    Id: clause.Id,
                    Name: clause.Name
                }));
                if (result.length > 0) {
                    this.billingCountry = result[0].Billing_Country__c;
                }
            })
            .catch(error => {
                this.showToast('Error', 'Error fetching clauses: ' + error.body.message, 'error');
            });
    }
    ...
    closeModal() {
        //console.log('Closing modal...'); // Debugging check
